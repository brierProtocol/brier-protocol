...
function CommentRow({ post: p, isReply, parentId, address, canReply, onLike, onDelete, onReply }: CommentRowProps) {
  ...
  return (
    <div className={isReply ? 'flex gap-2.5 pl-[52px] pt-3' : 'flex gap-3 px-5 py-4'}>
      <MakerAvatar address={p.wallet} pfpUrl={p.user?.pfpUrl} size={isReply ? 28 : 36} square />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 mb-1">
          <span className={`font-bold text-white ${isReply ? 'text-[12px]' : 'text-[13px]'}`}>{personLabel(p.user, p.wallet)}</span>
          <span className="font-mono text-[10px] text-[#48484f] tabular-nums">{relDay(p.createdAt) || 'now'}</span>
        </div>
        {p.text && <div className="text-[13px] text-[#cfcfd6] leading-relaxed"><PostBody text={p.text} onQuoteClick={() => {}} /></div>}
        {p.mediaUrl && (
          // eslint-disable-next-line @next/next/no-img-element -- data:/gif URLs, next/image can't optimize either
        ...
        {replying && (
          <div className="mt-2.5 flex gap-2">
            <MakerAvatar address={address} size={24} square />
            <div className="flex-1">
              <textarea value={replyText} onChange={e => setReplyText(e.target.value)} maxLength={500} placeholder={`Reply to ${personLabel(p.user, p.wallet)}…`}
                className="w-full h-[52px] bg-[#0a0a0c] border border-[#1f1f28] rounded-lg px-3 py-2 text-[12px] text-white outline-none focus:border-primary/50 resize-y placeholder:text-[#4a4a54]" />
