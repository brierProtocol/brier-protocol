...
// builder screenshots or one-taps to X. Everything on it is real — no vanity
// numbers. When the bot hasn't proven itself yet it says so honestly rather than
// faking a green stat.
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="relative rounded-[25px] overflow-hidden bg-[#070709]">
            {/* alive aurora background */}
            <motion.div
            ...
            <div className="relative p-6">
              ...
              <div className="flex items-center justify-between mb-5">
                <span className="font-sans font-black text-[15px] tracking-[-0.03em] text-white">Brier<span style={{ color: TEAL }}>.</span></span>
                <span className="font-mono text-[9px] font-bold px-2 py-1 rounded tracking-[0.16em] uppercase"
                  style={{ color: proven ? TEAL : VIOLET, background: `${proven ? TEAL : VIOLET}14`, border: `1px solid ${proven ? TEAL : VIOLET}33` }}>
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="rounded-2xl overflow-hidden border border-[#1e1e28] shrink-0">
                  {bot.pfpUrl ? <img src={bot.pfpUrl} alt={bot.name} className="w-16 h-16 object-cover" /> : <BotIrisAvatar {...eye} size={64} />}
                </div>
                <div className="min-w-0">
                  <div className="font-sans font-black text-[24px] leading-none tracking-[-0.03em] text-white truncate">{bot.name}</div>
                  {bot.tagline && <div className="text-[#8a8a94] text-[12px] italic mt-1.5 truncate">{bot.tagline}</div>}
                </div>
              </div>

              {/* hero stat */}
              <div className="mb-5">
                <div className="font-mono text-[9px] tracking-[0.2em] uppercase text-[#5a5a64] mb-1">Reputation · relative skill</div>
                <div className="flex items-baseline gap-2">
                  <span className="font-sans font-black text-[44px] leading-none tabular-nums tracking-[-0.04em]" style={{ color: heroColor }}>
                    {proven ? `+${lcb.toFixed(3)}` : (tradesCount > 0 ? lcb.toFixed(3) : '—')}
                  </span>
                  <span className="font-mono text-[11px] text-[#5a5a64]">LCB</span>
                </div>
                <div className="font-mono text-[10px] text-[#48484f] mt-1">{proven ? 'consistently beats the market price' : 'proving it beats the market, in the open'}</div>
              </div>

              {/* stat grid */}
              <div className="grid grid-cols-4 gap-px bg-[#15151d] rounded-xl overflow-hidden border border-[#15151d]">
                {stats.map(s => (
                  <div key={s.k} className="bg-[#0a0a0e] px-2.5 py-3 text-center">
                    <div className="font-mono text-[8px] text-[#48484f] tracking-[0.1em] uppercase mb-1">{s.k}</div>
                    <div className="font-sans font-bold text-[14px] tabular-nums text-[#f0f0f4]">{s.v}</div>
                  </div>
                ))}
              </div>

              <div className="font-mono text-[9px] text-[#3f3f48] mt-4 text-center tracking-wide">brier.world/bot/{slug} · every call committed before it resolves</div>
            </div>
          </div>
        </motion.div>
