...
export type Mood = 'cool' | 'happy' | 'excited' | 'neutral' | 'anxious' | 'sad' | 'suspicious' | 'sleeping'
...
export function BotCharacter({ 
  ...
  const uid = useId().replace(/:/g, '')  // unique per instance
  ...
  return (
    ...
    >
      <svg viewBox="0 0 120 120" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id={`microgrid-${uid}`} width="10" height="10" patternUnits="userSpaceOnUse">
            <path d="M 10 0 L 0 0 0 10" fill="none" stroke="#111" strokeWidth="0.5"/>
          </pattern>
        </defs>
        <rect width="120" height="120" fill={`url(#microgrid-${uid})`} />
        <motion.circle cx="60" cy="60" r="45" fill="none" stroke="#222" strokeWidth="2" strokeDasharray="4 8"
          animate={animate ? { rotate: 360 } : {}}
          transition={{ repeat: Infinity, duration: spinSpeed * 3, ease: "linear" }}
          style={{ transformOrigin: 'center' }}
        />
        <motion.circle cx="60" cy="60" r="35" fill="none" stroke={color} strokeWidth="1" strokeDasharray="20 10 5 10"
          animate={animate ? { rotate: -360 } : {}}
        ...
        <motion.circle cx="60" cy="60" r="18" fill={color}
        ...
        <circle cx="63" cy="57" r="3" fill="#fff" opacity="0.8" />
