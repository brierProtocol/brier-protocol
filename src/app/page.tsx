...
export default function ComingSoon() {
  ...
  const [aliens, setAliens] = useState<{ id: string, startX: number, startY: number, endX: number, endY: number, color: string, key: number, size: number, duration: number }[]>([])
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        {aliens.map(a => (
          <motion.div
            ...
            className="absolute drop-shadow-[0_0_20px_rgba(255,255,255,0.02)]"
          >
            <BotIrisAvatar avatarId={a.id} size={a.size} accentColor={a.color} />
          </motion.div>
        ))}
      </div>
