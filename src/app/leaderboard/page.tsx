      ...
      <div className="mx-auto max-w-7xl relative z-10">
        ...
        >
          ...
          <h1 className="font-[var(--font-display)] text-[64px] sm:text-[96px] font-black uppercase tracking-tighter text-white mb-6 leading-none drop-shadow-2xl">
            Protocol <span className="text-[#D4AF37]">Elite</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-12">
            ...
            <div className="relative w-full max-w-sm group">
              <div className="absolute inset-0 bg-[#00F0FF]/20 blur-xl opacity-0 group-focus-within:opacity-100 transition-opacity" />
              <input
                type="text"
        ...
        {isLoading ? (
          <div className="mb-24 grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-5xl mx-auto items-end pt-32">
             ...
             <div className="h-72 bg-white/5 rounded-[40px] animate-pulse border border-white/10 shadow-[0_0_40px_rgba(0,240,255,0.1)]" />
            ...
            {top3[0] && (
              <Link href={`/vault/${top3[0].id}`} className="block sm:order-2 transition-all hover:scale-[1.02] z-10 group">
                ...
                >
                  ...
                  <div className="mt-10 w-full bg-[#00F0FF]/5 rounded-[32px] p-6 border border-[#00F0FF]/10 grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      <p className="font-[var(--font-dm-mono)] text-3xl font-bold text-[#00F0FF] leading-none">{(top3[0].winRate * 100).toFixed(0)}%</p>
        ...
        <div className="flex flex-col gap-4 max-w-6xl mx-auto">
          {isLoading ? (
            ...
            filteredBots.slice(3).map((bot, i) => {
              const rank = i + 4;
              const wrColor = bot.winRate > 0.55 ? 'text-[#00F0FF]' : bot.winRate < 0.45 ? 'text-[#FF3D00]' : 'text-white';
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-8 flex-1 w-full">
                    <div className="font-[var(--font-dm-mono)] text-[28px] font-bold text-white/10 min-w-[60px] text-center group-hover:text-[#00F0FF]/20 transition-colors">
                      #{rank.toString().padStart(2, '0')}
                    </div>
                    <div className="flex items-center gap-6 flex-1">
                      <div className="w-14 h-14 rounded-2xl overflow-hidden flex items-center justify-center shrink-0 border border-white/10 group-hover:border-[#00F0FF]/30 transition-colors" style={{ background: bot.color + '20' }}>
                        <BotCharacter mood={bot.mood as any} accentColor={bot.color} size={40} animate={false} className="scale-90" />
                      </div>
                      <div>
                        <Link href={`/vault/${bot.id}`}>
                          <h3 className="font-[var(--font-display)] text-[22px] font-black uppercase text-white hover:text-[#00F0FF] transition-colors tracking-tight">
                            {bot.name}
                          </h3>
                        </Link>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="h-1.5 w-1.5 rounded-full bg-white/20 group-hover:bg-[#00F0FF] transition-colors" />
                  ...
                  <div className="w-full md:w-auto mt-4 md:mt-0">
                    <Link
                      href={`/vault/${bot.id}`}
                      className="inline-flex items-center justify-center w-full md:w-auto rounded-full bg-white text-[#050505] px-10 py-4 font-[var(--font-dm-mono)] text-[11px] font-black uppercase tracking-widest transition-all hover:bg-[#00F0FF] hover:scale-105 active:scale-95 shadow-2xl"
                    >
                      Open Vault
                    </Link>
                  </div>
